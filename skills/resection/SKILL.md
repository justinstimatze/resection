...
9. **Adjudicate flagged rows.** For each flagged row printed in step 8,
   ...
   side does)? Weigh headline-flagged rows especially carefully — check
   whether either side's `falsification_criterion` actually engaged with
   the substitute-vs-real question, or just restated the label match.

10. **Write the report.** `<rundir>/report.md`: the full claim table (all
    rows, not just flagged ones), then a clearly separated section with
    your adjudicated verdict on each flagged row and why. Include one
    caveat line: the claim list was authored by the same vision-reader
    dispatch that scored the vision side (step 3 and step 5 are the same
    agent type, not independently produced), so treat vision-side "done"
    verdicts with slightly more skepticism than implementation-side ones —
    this is a known, accepted v1 tradeoff (see docs/design.md), not an oversight.

11. **Summarize to the user.** State the flagged count, name the headline
    claims among them first, and give your adjudicated read on each. Point
    at `<rundir>/report.md` for the full table rather than pasting it whole.
