...
exactly the claim that mattered most — and a plain status-equality diff has no way to see the
difference between "we both verified this" and "we both took the same shortcut," because that
difference lives in prose, not in the status field. The claim schema's `headline` and
`falsification_criterion` fields make each side state its reasoning; `resection validate`
mechanically enforces that the field is non-empty on a headline claim, but presence isn't
substance, and substance is a judgment call no Go equality check can make. That judgment call is
made explicitly, by the orchestrating session, for every headline claim — matched or flagged —
in the skill's own adjudication step (`skills/resection/SKILL.md` step 9a), specifically because
a matched headline claim never appears anywhere in the diff's own output otherwise (see
[README.md](../README.md#what-each-side-can-and-cant-catch) for the field-level detail and its
own stated limits).
